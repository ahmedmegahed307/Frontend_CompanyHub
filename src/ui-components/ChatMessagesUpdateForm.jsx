...
export default function ChatMessagesUpdateForm(props) {
  const {
    id: idProp,
    chatMessages: chatMessagesModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = chatMessagesRecord
      ? { ...initialValues, ...chatMessagesRecord }
      : initialValues;
    setText(cleanValues.text);
    setSenderId(cleanValues.senderId);
    setReciverId(cleanValues.reciverId);
    setIsRead(cleanValues.isRead);
    setIsActive(cleanValues.isActive);
    setErrors({});
  };
  const [chatMessagesRecord, setChatMessagesRecord] = React.useState(
    chatMessagesModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(ChatMessages, idProp)
        : chatMessagesModelProp;
      setChatMessagesRecord(record);
    };
    queryData();
  }, [idProp, chatMessagesModelProp]);
  React.useEffect(resetStateValues, [chatMessagesRecord]);
  const validations = {
    text: [],
    senderId: [],
    reciverId: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            ChatMessages.copyOf(chatMessagesRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ChatMessagesUpdateForm")}
      {...rest}
    >
      <TextField
        label="Text"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || chatMessagesModelProp)}
